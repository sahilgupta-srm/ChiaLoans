...
updaterepayed(id,lender,owner,amount)
  {
  let that=this;
  console.log("update repayed",id);
return new Promise((resolve, reject) => {
      let paymentContract = TruffleContract(tokenAbi2);
      paymentContract.setProvider(that.web3Provider);
      paymentContract.deployed().then(function(instance) {
var value=web3.eth.sendTransaction({from: window.web3.eth.accounts[0], to: owner, value: web3.toWei(amount, "ether"), gas: "313282"}
, function(err, transactionHash) {
  if (!err)
    console.log(transactionHash); 
});
          return instance.updaterepayed(id,{from: window.web3.eth.accounts[0]});
        }).then(function(status) {
          if(status) {
            return resolve(status);
          }
